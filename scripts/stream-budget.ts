...
console.log('preset          buffer      zoom  radius  chunk  tekstur  instance  draw');
...
for (const id of PRESET_IDS) {
  ...
  const plan = planDisplay(cssW, cssH, dpr, p.pixelHeight);
  const cam = new IsoCamera();
  cam.setViewport(plan.width, plan.height);
  ...
  console.log(
    `${p.name.padEnd(14)} ${`${plan.width}x${plan.height}`.padEnd(11)} ${String(plan.zoom).padEnd(5)} ` +
